...
interface StepSubmitProps extends React.HTMLAttributes<HTMLInputElement> {
  currentStep: number
  onBack?: () => void
  onNext?: () => void
}

const StepSubmit = forwardRef<HTMLInputElement, StepSubmitProps>(({ currentStep, onBack, onNext, className }, ref) => {
  const [setPersoon] = useRsvpStore((state) => [state.setPersoon])
...
StepSubmit.displayName = 'Step submit'
